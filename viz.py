...
def _lag_axis(shifts, bin_width):
    return shifts * bin_width if bin_width else shifts


def plot_kernels(W, names, bin_width=None, mean_sem=False, axes=None, figsize=None):
    """Plot each predictor's fitted kernel vs lag. W (P, U), names '{pred}_{shift}'.
    bin_width: seconds per bin (x-axis in seconds if given, else in bins).
    ...
    for ax, pred in zip(axes, preds):
        ...
        ax.set_title(pred); ax.set_xlabel("lag (s)" if bin_width else "lag (bins)")
...
def plot_reconstruction(Y, mu, unit=0, window=None, bin_width=None, ax=None):
    ...
    t = np.arange(a, b) * (bin_width or 1.0)
    ...
    ax.set_xlabel("time (s)" if bin_width else "bin"); ax.set_ylabel(f"unit {unit}")
...
def plot_event_reconstruction(Y, mu, event_idx, unit=0, pre=20, post=40, bin_width=None, ax=None):
    ...
    ax.set_xlabel("lag (s)" if bin_width else "lag (bins)"); ax.set_ylabel(f"unit {unit}")
