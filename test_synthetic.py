...
def irls_ridge_one(X, y, l2, iters=100):
    ...
    for _ in range(iters):
        ...
        grad = Xa.T @ (mu - y) + Rpen @ beta
        H = Xa.T @ (Xa * mu[:, None]) + Rpen
...
def main():
    ...
    l2 = 5.0
    Wz2, bz2, _, _ = pg.fit_units(Xz, Yj, alpha=l2, l1_ratio=0.0, L0=1.0,
                                  max_iter=3000, tol=1e-11)
    ...
    assert max_mu_err < 1e-3, "FISTA ridge fit disagrees with IRLS ground truth"

    # ---- 3. L1 induces sparsity
    Wz_hi, _, _, _ = pg.fit_units(Xz, Yj, alpha=0.2, l1_ratio=1.0, L0=1.0,
